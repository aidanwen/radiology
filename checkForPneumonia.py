...
imagePaths = os.listdir('stage_1_train_images')
...
for (i, imagePath) in enumerate(imagePaths[:1000]):
	...
	img = pydicom.dcmread('stage_1_train_images/' + imagePath).pixel_array
...
with open('stage_1_train_labels.csv') as csv_file:
	...
	for row in csv_reader:
		if line_count == 0:
			...
			height = row[4]
			target = row[5]
			labels.append(target) #TODO: Figure out label format / how to change it
			line_count += 1
